...
3. 📝 **Lock paper draft** — update CICIDS2017 section per item 2; report per-cluster clean accuracy and the single-class cliff (audit report §4) instead of pooled means
4. 🔬 **Attack generalization (baseline report §Future Work)** — implemented, ready to run: `app/ml/attacks/cw.py` (Carlini–Wagner adapted to the mixed-norm threat model: ε-bounded continuous perturbation via `δ = ε·tanh(p)`, f6 margin loss, per-sample binary search on c; K-flip categorical enumeration as in the canonical attack) + `scripts/eval_cw.py` (results → `results/cw/`) and `scripts/eval_foolbox.py` (independent Foolbox 3.3.4 reference attacks CW-L2/DeepFool/PGD on the continuous-column subspace, results → `results/foolbox/`). Cross-validated on NSL-KDD/RSC: own CW 86.1% vs Foolbox CW-L2 85.9% robust accuracy (clean 88.1%).
