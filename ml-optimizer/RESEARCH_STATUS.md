...
**K=1 exhaustive C&W (rsc_cicids2017_seed53) — running, evidence so far:** budget-convergence verified at K=1 same as K=0 (survivors identical across 300×3 → 600×6 → 600×6+c×100). **Budget-utilization finding (paper-worthy, own sentence):** successful CW perturbations use ~0.05–0.097 of the 0.15 L∞ ball at K=0 but saturate it (~0.149–0.150) at K=1 — independent of survivor counts, this shows flip states unlock samples continuous-only optimization cannot reach because the combined attack genuinely requires the full budget. Methods text must note: the K=1 probe ran on a similar-but-not-identical batch to the main run (460 vs 458 clean-correct; RNG/streaming-loader ordering) — byte-identical inputs across probe variants is what the within-probe convergence claim rests on. Early-batch nesting pattern (cw_only=0 through batch ~108) pending full-set confirmation; PGD-side masks carry ±~0.3pp RNG jitter (quantified in NSL-KDD survivor study), CW side deterministic given data.
